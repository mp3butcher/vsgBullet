...
void DragHandler::apply(vsg::MoveEvent& moveEvent)
{
    ...
    if(_ctrlpressed&&_constraint)
    {
        ...
        vsg::vec4 farPointCC = vsg::vec4( 1., 1., 1., 1. );
        farPointCC*= proj->farDistance;
        vsg::vec4 farPointWC = farPointCC * vsg::mat4(lookat->inverse()*proj->inverse()  );
        ...
        double length = -( vsg::dot(planeNormal, lookat->center) + _dragPlane[ 3 ] ) / dotVd;
        vsg::vec3 pointOnPlane = vsg::vec3(lookat->center);
        pointOnPlane +=  vsg::vec3( vDir );
        pointOnPlane *= length;
        ...
        vsg::vec3 bulletPoint =  pointOnPlane* ow2bw ;
    ...
    for (auto& intersection : intersector->intersections)
    {
        ...
        for (auto& node : intersection->nodePath)
        {
            ...
            if( rb.valid())
            {
                ...
                vsg::vec3 pickPointBulletOCLocal = vsg::vec3(pickPointWC) * ow2col;
                //   vsg::notify( vsg::DEBUG_FP ) << "pickPointWC: " << pickPointWC << std::endl;
                //     vsg::notify( vsg::DEBUG_FP ) << "pickPointBulletOCLocal: " << pickPointBulletOCLocal << std::endl;
