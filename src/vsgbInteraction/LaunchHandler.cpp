...
void LaunchHandler::apply(vsg::ButtonPressEvent& buttonPress)
{
    ...
    if(_ctrlpressed){
        ...
        vsg::ref_ptr< vsgbDynamics::RigidBody > amt =  vsgbDynamics::RigidBody::create();
        amt->addChild( _launchModel );

        vsg::ref_ptr<vsg::Viewer> viewer(_refviewer);
        viewer->addUpdateOperation(vsgbCollision::opMerge::create(_refviewer, _attachPoint, amt, viewer->compileManager->compile(amt)));    
...
void LaunchHandler::reset()
{
    ...
    if(_world.valid() &&   ( ddw=_world->getDynamicsWorld()) )
        for( it=_nodeList.begin(); it != _nodeList.end(); ++it )
        {
            ...
            vsgbCollision::RefRigidBody* rrb = NULL;// dynamic_cast< vsgbCollision::RefRigidBody* >( node->getUserData() );
            if( rrb == NULL )
            {
