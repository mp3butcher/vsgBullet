...
ReaderWriterSGB::readObject( const std::string& fileName, const Options* options ) const
{
   ...
   /* vsg::ref_ptr< osgbDynamics::PhysicsState > ps = new osgbDynamics::PhysicsState;
    unsigned int idx;
    for( idx = 0; idx < numEntries; idx++ )
    {
        ...
        vsg::ref_ptr< osgbDynamics::PhysicsData > pd = static_cast< osgbDynamics::PhysicsData* >( fr.readObject() );
        if( pd == NULL )
        {
