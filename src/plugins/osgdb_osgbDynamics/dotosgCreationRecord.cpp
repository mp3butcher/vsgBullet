...
bool Creation_readLocalData( vsg::Object& obj, osgDB::Input& fr );
bool Creation_writeLocalData( const vsg::Object& obj, osgDB::Output& fw );

osgDB::RegisterDotOsgWrapperProxy Creation_Proxy
(
    new vsgbDynamics::CreationRecord,
    "CreationRecord",
...
bool Creation_readLocalData( vsg::Object& obj, osgDB::Input& fr )
{
    vsgbDynamics::CreationRecord& cr = static_cast< vsgbDynamics::CreationRecord& >( obj );
    bool advance( false );
    ...
    else if( fr.matchSequence( "VertexAgg min cell size %f %f %f" ) )
    {
        ...
        fr[5].getFloat( y );
        fr[6].getFloat( z );
        cr._vertexAggMinCellSize = vsg::vec3( x, y, z );
        */
        fr+=7;
        advance = true;
    }
    ...
    else if( fr.matchSequence( "Cylinder axis %i" ) )
    {
        unsigned int uint;
        fr[2].getUInt( uint );
        cr._axis = (vsgbCollision::AXIS)( uint );
        fr+=3;
        advance = true;
    }
    else if( fr.matchSequence( "Reduction level %i" ) )
    {
        unsigned int uint;
        fr[2].getUInt( uint );
        cr._reductionLevel = (vsgbDynamics::CreationRecord::ReductionLevel)( uint );
        fr+=3;
        advance = true;
    }
...
bool Creation_writeLocalData( const vsg::Object& obj, osgDB::Output& fw )
{
    const vsgbDynamics::CreationRecord& cr = static_cast< const vsgbDynamics::CreationRecord& >( obj );
