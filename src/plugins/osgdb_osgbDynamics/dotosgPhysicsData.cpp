...
bool PhysicsData_readLocalData( osg::Object& obj, osgDB::Input& fr )
{
    vsgbDynamics::PhysicsData& pd = static_cast< vsgbDynamics::PhysicsData& >( obj );
    bool advance( false );
    ...
    if( fr.matchSequence( "Version %i" ) )
    {
        ...
        pd._cr = static_cast< vsgbDynamics::CreationRecord* >( fr.readObject() );
...
bool PhysicsData_writeLocalData( const osg::Object& obj, osgDB::Output& fw )
{
    const vsgbDynamics::PhysicsData& pd = static_cast< const vsgbDynamics::PhysicsData& >( obj );

    fw.indent() << "Version " << pd.getVersion() << std::endl;

    if( pd._cr == nullptr )
        osg::notify( osg::WARN ) << "PhysicsData_writeLocalData: Can't write nullptr CreationRecord." << std::endl;
    else
    ...
    vsgbDynamics::MotionState* ms = dynamic_cast< vsgbDynamics::MotionState* >( motion );
    if( ms != nullptr )
    {
        osg::Transform* trans = ms->getTransform();
        if( trans->asMatrixTransform() != nullptr )
        {
    ...
    osg::Matrix m( vsgbCollision::asOsgMatrix( pd._body->getWorldTransform() ) );
    writeMatrix( m, fw, "BodyWorldTransform" );
    osg::Vec3 lv( vsgbCollision::asOsgVec3( pd._body->getLinearVelocity() ) );
    fw.indent() << "Linear velocity " << lv << std::endl;
    osg::Vec3 av( vsgbCollision::asOsgVec3( pd._body->getAngularVelocity() ) );
