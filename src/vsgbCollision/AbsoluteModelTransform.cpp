...
bool
AbsoluteModelTransform::computeLocalToWorldmat4( vsg::mat4& mat4, vsg::NodeVisitor* nv ) const
{
    if( getReferenceFrame() == vsg::Transform::ABSOLUTE_RF )
    {
        vsg::mat4 view;
        if( !nv )
            vsg::notify( vsg::INFO ) << "AbsoluteModelTransform: NULL NodeVisitor; can't get view." << std::endl;
...
bool
AbsoluteModelTransform::computeWorldToLocalmat4( vsg::mat4& mat4, vsg::NodeVisitor* nv ) const
{
    ...
    if( getReferenceFrame() == vsg::Transform::ABSOLUTE_RF )
    {
        vsg::mat4 invView;
        if( !nv )
            vsg::notify( vsg::NOTICE ) << "AbsoluteModelTransform: NULL NodeVisitor; can't get invView." << std::endl;
        else if( nv->getVisitorType() != vsg::NodeVisitor::CULL_VISITOR )
