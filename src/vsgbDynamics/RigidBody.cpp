...
{
    const   vsgbCollision::RefBulletObject<  btRigidBody >*  refptr=
        dynamic_cast<  const   vsgbCollision::RefBulletObject<  btRigidBody >* >( getUserData() );
...
{
    vsgbCollision::RefBulletObject<  btRigidBody >*  refptr=
        dynamic_cast<     vsgbCollision::RefBulletObject<  btRigidBody >* >( getUserData() );
...
{
    setUserData(new vsgbCollision::RefBulletObject<btRigidBody>(b));
    //btRigidBody >* rb = dynamic_cast<        vsgbCollision::RefBulletObject<  >* >();
...
btRigidBody* createRigidBody( vsgbDynamics::CreationRecord* cr )
{
    ...
    if( root == NULL )
    {
        std::cerr << "createRigidBody: CreationRecord has NULL scene graph." << std::endl;
        return( NULL );
    }
/***
    vsg::dbox bs = root->getBound();
    ...
    vsg::mat4 m( vsg::translate( -com ) * vsg::scale( cr->_scale ) );
    vsg::ref_ptr< vsg::MatrixTransform > tempMtRoot =  vsg::MatrixTransform::create( m );
    ...
    if( shape == NULL )
    {
        std::cerr << "createRigidBody: btCompoundShapeFromVSGGeodes returned NULL." << std::endl;
        return( NULL );
    }
...
if( root == NULL )
  ...
  //      com = root->getBound().center();
    ...
    if( rb == NULL )
    {
        std::cerr << "createRigidBody: Created a NULL btRigidBody." << std::endl;
        return( NULL );
    }
