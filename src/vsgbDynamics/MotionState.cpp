...
void
MotionState::setTransform( vsg::Transform* transform )
{
    auto mt=transform->cast<vsg::MatrixTransform>();
    vsg::ref_ptr<vsg::AbsoluteTransform> amt;
    if( mt != NULL )
        _mt = mt;
    else if( (amt = transform->cast<vsg::AbsoluteTransform>()) != NULL )
        _amt = amt;
    else
...
void
MotionState::setParentTransform( const vsg::dmat4 m )
{
//    vsg::notify( vsg::ALWAYS) << "setParent" << m << std::endl;
    vsg::dmat4 d;
    vsgbCollision::orthoNormalize<double>(d,m);
    _parentTransform = d;// vsg::mat4::orthoNormal( m );
...
void
MotionState::setCenterOfMass( const vsg::vec3& com )
{
    _com = com;
    resetTransform();
}
...
void
MotionState::setScale( const vsg::vec3& scale )
{
    _scale = scale;
    resetTransform();
}
...
void
MotionState::registerTripleBuffer( vsgbDynamics::TripleBuffer* tb )
{
    _tb = tb;
