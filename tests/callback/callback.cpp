...
/* \cond */
struct MyCallback : public vsgbDynamics::MotionStateCallback
{
    unsigned int _count;
              ...
              vsgbInteraction::SaveRestoreHandler* srh )
{
    ...
    model->setUserData( new vsgbCollision::RefRigidBody( rb ) );
    srh->add( "box", rb );
}
...
int main( int argc, char** argv )
{
    ...
    osg::ref_ptr< vsgbInteraction::SaveRestoreHandler > srh = new
        vsgbInteraction::SaveRestoreHandler;

    osg::Group* launchHandlerAttachPoint = new osg::Group;
    root->addChild( launchHandlerAttachPoint );

    root->addChild( vsgbDynamics::generateGroundPlane( osg::Vec4( 0.f, 0.f, 1.f, -10.f ), bw ) );
    ...
    viewer.addEventHandler( new vsgbInteraction::DragHandler( bw, viewer.getCamera() ) );
