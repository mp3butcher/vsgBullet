...
struct Joint
{
    ...
    void set( float angle )
    {
        ...
        if (_btChildIdx >= 0)
            _artShape->updateChildTransform( _btChildIdx, vsgbCollision::asBtTransform( btm ) );
            //_artShape->getChildList()[ _btChildIdx ].m_transform
              //  = vsgbCollision::asBtTransform( btm );
...
{
public:
    BulletShapeVisitor() : _shape( nullptr ) {}
    ...
    virtual void apply( const osg::Sphere& s )
    {
        ...
        btTransform xform;
        xform.setIdentity();
        xform.setOrigin( vsgbCollision::asBtVector3( c ) );
...
public:
    ...
    void apply( osg::Group& node )
    {
        ...
        btTransform xform =
            vsgbCollision::asBtTransform( osg::Matrix::translate( bb.center() ) );
    ...
    void apply( osg::Geode& node )
    {
        ...
        for ( idx=0; idx < node.getNumDrawables(); idx++ )
        {
            ...
            if (shape)
            {
                ...
                if (bsv._shape)
                {
                    vsgbCollision::RefBulletObject< btCollisionShape >* collision =
                        new vsgbCollision::RefBulletObject< btCollisionShape >( bsv._shape );
    ...
    btCollisionShape* accumulateChildCollisionShapes( osg::Group& node )
    {
        ...
        for ( idx=0; idx < node.getNumChildren(); idx++ )
        {
            ...
            if ( dynamic_cast< osg::MatrixTransform* >( child ) )
                continue;
            vsgbCollision::RefBulletObject< btCollisionShape >* bcs = dynamic_cast<
                vsgbCollision::RefBulletObject< btCollisionShape >* >( child->getUserData() );
            if (!bcs)
                continue;
...
createBall( btDynamicsWorld* dynamicsWorld )
{
    ...
    osg::Node* debugNode = vsgbCollision::osgNodeFromBtCollisionShape( collision );
