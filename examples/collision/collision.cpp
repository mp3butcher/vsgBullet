...
public:
    ...
    virtual bool handle( const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa )
    {
        ...
        else if( ea.getEventType() == osgGA::GUIEventAdapter::DRAG )
        {
            ...
            deltaX *= 6.;
            deltaY *= 6.;
            vsg::mat4 trans = osgbCollision::asOsgMatrix( _co->getWorldTransform() );
        ...
        for( unsigned int i = 0; i < numManifolds; i++ )
        {
            ...
            for( unsigned int j=0; j<numContacts; j++ )
            {
                ...
                if( ( pt.getDistance() <= 0.f ) && ( lastColState == false ) )
                {
                    ...
                    vsg::vec3 pos = osgbCollision::asOsgVec3( pt.getPositionWorldOnA() ); // position of the collision on object A
                    vsg::vec3 normal = osgbCollision::asOsgVec3( pt.m_normalWorldOnB ); // returns a unit vector
