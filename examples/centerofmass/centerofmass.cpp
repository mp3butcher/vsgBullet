...
{
    vsg::Node* node = osgDB::readNodeFile( "com.osg" );
    if( node == NULL )
    {
        std::cerr << "Can't load file \"com.osg\". Make sure osgBullet data directory is in OSG_FILE_PATH." << std::endl;
        return( NULL );
    }
...
int main( int argc, char** argv )
{
   ...
   // vsg::ref_ptr< osgbInteraction::SaveRestoreHandler > srh = new        osgbInteraction::SaveRestoreHandler;
    ...
    m = vsg::mat4::rotate( .4, 0., 0., 1. ) * vsg::mat4::translate( -24., 0., 10. );
   ...
   , vsg::vec3( 0., 0., 0. ), true ) );
    ...
    m = vsg::mat4::rotate( .4, 0., 0., 1. ) * vsg::mat4::translate( -4., 0., 10. );
    ...
    m = vsg::mat4::rotate( .4, 0., 0., 1. ) * vsg::mat4::translate( 16., 0., 10. );
    bw->addRigidBody( createObject( root, m    /*, srh.get()*/ ) );

    root->addChild( osgbDynamics::generateGroundPlane( vsg::Vec4( 0.f, 0.f, 1.f, 0.f ), bw ) );
