...
#ifdef USE_P5
#include <vsgbInteraction/p5support.h>
#endif
...
{
public:
    GestureTest( vsgbInteraction::HandNode* handNode ) : _handNode( handNode ) {}
    ...
    bool handle( const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& )
    {
        ...
        case osgGA::GUIEventAdapter::KEYUP:
        {
            if( (ea.getKey() == 'd' ) || (ea.getKey() == 'D' ) )
            {
                ...
                // Default gesture
                _handNode->sendGestureCode( vsgbInteraction::GestureHandler::Default );
                _handNode->setPose( vsgbInteraction::HandNode::POSE_DEFAULT );
                return( true );
            }
            else if( (ea.getKey() == 'p' ) || (ea.getKey() == 'P' ) )
            {
                // Point gesture
                _handNode->sendGestureCode( vsgbInteraction::GestureHandler::Point );
                _handNode->setPose( vsgbInteraction::HandNode::POSE_POINT );
                return( true );
            }
            else if( (ea.getKey() == 'f' ) || (ea.getKey() == 'F' ) )
            {
                ...
                // Fist gesture
                _handNode->sendGestureCode( vsgbInteraction::GestureHandler::Fist );
                _handNode->setPose( vsgbInteraction::HandNode::POSE_FIST );
                return( true );
            }
        }
        }
...
protected:
    osg::ref_ptr< vsgbInteraction::HandNode > _handNode;
};
/** \endcond */
...
btDiscreteDynamicsWorld* initPhysics( osg::Vec3 gravity = osg::Vec3( 0, 0, -9.8 ) )
{
    ...
    dynamicsWorld->setGravity( vsgbCollision::asBtVector3( gravity * 9.8 ) );
...
{
public:
    ConfigReaderWriter( osg::Group* root, btDiscreteDynamicsWorld* dw, vsgbInteraction::HandNode* hn )
      : _up( osg::Vec3( 0, 0, 1 ) ),
        _root( root ),
        _dw( dw ),
        _hn( hn )
    ...
    bool read( const std::string& fileName )
    {
        ...
        while( !in.eof() )
        {
            ...
            {
                istr >> _up;
                _dw->setGravity( vsgbCollision::asBtVector3( _up * -9.8 ) );
            }
            ...
            else if( key == std::string( "HandNode:" ) )
            {
                ...
                else
                {
                    ...
                    _hn->setHandedness( right ? vsgbInteraction::HandNode::RIGHT : vsgbInteraction::HandNode::LEFT );
                    _hn->setPosition( pos );
            ...
            else if( key == std::string( "Hinge:" ) )
            {
                ...
                const btVector3 btPivotA( vsgbCollision::asBtVector3( pivotA ) );
                const btVector3 btPivotB( vsgbCollision::asBtVector3( pivotB ) );
                btVector3 btAxisA( vsgbCollision::asBtVector3( axisA ) );
                btVector3 btAxisB( vsgbCollision::asBtVector3( axisB ) );
            ...
            else if( key == std::string( "Slider:" ) )
            {
                ...
                vsgbCollision::RefRigidBody* rbA = dynamic_cast< vsgbCollision::RefRigidBody* >( amtA->getUserData() );
                if( rbA == nullptr )
                {
                ...
                osg::Matrix m;
                amtA->computeLocalToWorldMatrix( m, nullptr );
                m = m * axisMatrix;
                btMatrix3x3 m3A( vsgbCollision::asBtMatrix3x3( m ) );
                ...
                for( idx=0; idx<fnnB._napl.size(); idx++ )
                {
                    ...
                    vsgbCollision::RefRigidBody* rbB = dynamic_cast< vsgbCollision::RefRigidBody* >( amtB->getUserData() );
                    if( rbB == nullptr )
                    {
                    ...
                    bs = amtB->getBound();
                    osg::Vec3 centerB = bs.center();
                    btVector3 offsetA( vsgbCollision::asBtVector3( centerB - centerA ) );

                    // Get the 3x3 basis of body B's reference frame.
                    amtB->computeLocalToWorldMatrix( m, nullptr );
                    m = m * axisMatrix;
                    btMatrix3x3 m3B( vsgbCollision::asBtMatrix3x3( m ) );
            ...
            else if( key == std::string( "Constraint:" ) )
            {
                ...
                osg::ref_ptr< vsgbDynamics::Constraint > cons = static_cast< vsgbDynamics::Constraint* >(
        ...
        if( !noGroundPlane )
        {
            ...
            if( groundFilters == osg::Vec2s( 0, 0 ) )
                groundNode = vsgbDynamics::generateGroundPlane( groundPlane, _dw );
            else
                groundNode = vsgbDynamics::generateGroundPlane( groundPlane, _dw, nullptr, filterGroup, filterWith );
    ...
    bool write( const std::string& fileName )
    {
        ...
        out << "HandNode: " << _hn->getPosition() << " " <<
            _hn->getAttitude() << " " <<
            _hn->getHandLength() << " " <<
            (_hn->getHandedness() == vsgbInteraction::HandNode::RIGHT) << std::endl;
...
protected:
    ...
    btRigidBody* createRigidBody( osg::Node* subgraph, const osg::NodePath& np, vsgbDynamics::CreationRecord* cr )
    {
        ...
        btRigidBody* rb = vsgbDynamics::createRigidBody( cr );
        if( rb == nullptr )
        {
            osg::notify( osg::WARN ) << "Warning: createRigidBody: nullptr rigid body." << std::endl;
            return( nullptr );
        }
    ...
    osg::ref_ptr< vsgbInteraction::HandNode > _hn;
};
/** \endcond */
...
int main( int argc, char** argv )
{
    ...
    osg::ref_ptr< vsgbInteraction::HandNode > hn = new
        vsgbInteraction::HandNode( bulletWorld );
    ...
    hn->getGestureHandlerVector().push_back( new vsgbInteraction::GripRelease );
...
#ifdef USE_P5
    vsgbInteraction::P5Glove* gloveZero = new vsgbInteraction::P5Glove( true );
...
to interact with the scene using the vsgbInteraction::HandNode.
...
For keyboard controls, see vsgbInteraction::VirtualHandTestEventHandler.
